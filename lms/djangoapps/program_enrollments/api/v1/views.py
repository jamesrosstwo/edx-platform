...
class UserProgramReadOnlyAccessView(DeveloperErrorViewMixin, PaginatedAPIView):
    ...
    def get(self, request):
        ...
        program_type_filter = request.GET.get('type', self.DEFAULT_PROGRAM_TYPE)

        if request_user.is_staff:
            all_programs = get_programs(request.site)
            catalog_data_of_programs = [program 
                for program in all_programs 
                if program['type'].lower() == program_type_filter]
    ...
    def get_programs_user_is_course_staff(self, user, program_type_filter):
        """
